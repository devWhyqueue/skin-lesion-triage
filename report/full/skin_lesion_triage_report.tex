...
The split protocol is lesion-aware. HAM10000 includes a \texttt{lesion\_id} field because a single physical lesion may have multiple images. If such images are split independently, visually similar observations of the same lesion can appear in both training and evaluation partitions, inflating performance. The analysis therefore uses grouped splitting so that each \texttt{lesion\_id} belongs to exactly one partition, and an explicit all-pairs check confirms that no \texttt{lesion\_id} appears in more than one partition. The partitions are produced by three successive stratified, grouped five-fold splits: one 20\% fold is held out as the test set, a further fifth of the remainder becomes the calibration set, and a further fifth of what is left becomes the validation set, leaving the rest for fitting. This yields roughly a 52/12/16/20 fit/validation/calibration/test division, in which each evaluation partition is a standard-sized fold rather than a hand-tuned ratio, large enough for stable AUC, calibration, and threshold estimates. The executed split contains 4,750 fit images, 1,146 validation images for checkpoint selection, 1,418 calibration images for Platt scaling and threshold selection, and 1,860 held-out test images for final evaluation. The melanoma prevalence is 13.2\% in the fit split, 12.5\% in validation, 10.0\% in calibration, and 10.8\% in test.
...
The classical and deep pipelines first differ in ranking performance. Averaged over three seeds, the HSV GMM reaches an AUC of $0.796 \pm 0.009$ on the held-out test split, whereas the EfficientNet-B0 MC Dropout predictive mean reaches $0.858 \pm 0.002$; the deep model is both higher and markedly more stable across seeds. The ROC curves in Figure~\ref{fig:roc} (primary seed, AUC $0.858$ and $0.805$) show that the deep model ranks lesions consistently better than global colour histograms, although the separation is still short of a clinically definitive diagnostic test.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\textwidth]{roc_curve_triage.png}
\caption{Receiver operating characteristic curves for the Platt-calibrated deep model and the HSV GMM baseline on the held-out test split (primary seed). The deep model has stronger ranking performance, but ROC AUC alone does not specify the triage operating point.}
\label{fig:roc}
\end{figure}

The training trajectory supports the interpretation that the deep model learned useful discriminative structure. On the primary seed, validation AUC improved from 0.57 after the first head-training epoch to a peak of 0.876 during fine-tuning. On the held-out test split, the calibrated Brier score was $0.079 \pm 0.006$ and test ECE was $0.019 \pm 0.006$, improved from a raw ECE of $0.026 \pm 0.009$. These values indicate that calibration made the probabilities more usable for decision-theoretic thresholding, although the threshold behavior below shows that ranking and calibration do not remove the referral tradeoff.

\begin{table}[htbp]
\centering
\caption{Model comparison on the held-out test split. AUC is reported as mean (standard deviation) over three seeds; both models are Platt-calibrated on the calibration split, and the GMM is fit on a fit-split subsample for speed. Both models are evaluated on the same test lesions.}
\label{tab:model-comparison}
\begin{tabularx}{\textwidth}{Y Y c Y}
\toprule
Model & Main feature representation & AUC & Calibration notes \\
\midrule
HSV GMM & Global HSV histograms & 0.796 (0.009) & Platt-calibrated generative posterior; test ECE 0.025, from 0.160 uncalibrated \\
EfficientNet-B0 + MC Dropout & Learned dermoscopic morphology & 0.858 (0.002) & Platt-calibrated probabilities; Brier 0.079, ECE 0.019 \\
\bottomrule
\end{tabularx}
\end{table}

The calibration summary in Figure~\ref{fig:calibration} explains why the decision analysis cannot rely on AUC alone. The reliability diagram compares predicted melanoma probability with empirical melanoma frequency on the held-out test split. Calibration is important operationally: the theoretical $10:1$ threshold refers about 40\% of test lesions rather than collapsing into universal referral, but it still leaves a substantial number of missed melanomas.
...
The central effect of asymmetric loss is visible in Table~\ref{tab:thresholds}. At the symmetric threshold of 0.5, specificity is high but sensitivity is only 21.7\%, which is clinically unacceptable for melanoma screening. Lowering the threshold according to a $10:1$ loss ratio raises sensitivity to 87.3\% and keeps specificity at 65.6\%, referring 40.5\% of test lesions. A still more conservative $20:1$ threshold raises sensitivity to 94.4\% at the cost of a 53.6\% referral rate. The theoretical $10:1$ threshold therefore gives a manageable referral burden, but it still misses on average 27 of the roughly 200 melanomas in the held-out test split.

\begin{table}[htbp]
\centering
\caption{Held-out test triage metrics for the deep model under theoretical Bayesian thresholds from asymmetric loss. Thresholds are applied to Platt-calibrated MC Dropout predictive means. Values are mean (standard deviation) over three seeds; the threshold is fixed by the cost ratio.}
...
\midrule
1:1  & 0.500 & 0.217 (0.023) & 0.984 (0.002) & 0.644 (0.004) & 0.039 (0.004) & 168 (20) \\
5:1  & 0.167 & 0.760 (0.038) & 0.793 (0.020) & 0.325 (0.021) & 0.270 (0.025) & 51 (9) \\
10:1 & 0.091 & 0.873 (0.031) & 0.656 (0.036) & 0.250 (0.025) & 0.405 (0.037) & 27 (5) \\
20:1 & 0.048 & 0.944 (0.004) & 0.517 (0.046) & 0.204 (0.018) & 0.536 (0.044) & 12 (2) \\
\bottomrule
\end{tabular}
\end{table}

Because a theoretical loss ratio does not automatically guarantee a target clinical sensitivity on finite data, the analysis also selects thresholds on the calibration split by requiring minimum sensitivity and then evaluates them once on the held-out test split. Table~\ref{tab:operating-points} shows that a calibration-selected threshold for at least 98\% sensitivity gives 98.9\% test sensitivity, 27.1\% specificity, and roughly two false negatives. Requiring at least 99\% calibration sensitivity reaches 99.3\% test sensitivity with, on average, between one and two false negatives while referring 81.0\% of lesions. These operating points are more clinically interpretable than a single cost ratio because they state the intended safety level directly and expose how much referral burden is needed.

\begin{table}[htbp]
\centering
\caption{Sensitivity-constrained thresholds for the deep model, selected on the calibration split and evaluated on the held-out test split. Each row chooses the highest-specificity calibration threshold satisfying the target sensitivity. Values are mean (standard deviation) over three seeds.}
...
\midrule
0.95 & 0.036 (0.014) & 0.964 (0.044) & 0.451 (0.010) & 0.596 (0.044) & 8 (2) \\
0.98 & 0.015 (0.006) & 0.989 (0.035) & 0.271 (0.038) & 0.760 (0.028) & 2 (1) \\
0.99 & 0.010 (0.004) & 0.993 (0.029) & 0.214 (0.034) & 0.810 (0.028) & 1 (1) \\
\bottomrule
\end{tabular}
\end{table}
...
The experiment illustrates the distinction between discrimination, calibration, and decision-making. The deep model discriminates clearly better than the HSV GMM (AUC $0.858$ versus $0.796$), using spatial morphology that a global colour histogram discards. Better AUC still does not by itself solve the clinical problem. The Bayesian rule asks a different question: given a calibrated probability and an explicit loss matrix, which action minimizes expected harm? With unweighted training and Platt calibration, the predicted probabilities become usable for this decision rule: ECE is $0.019$, and the $10:1$ threshold produces a referral rate of about 40\% rather than collapsing into universal referral.

The calibrated $10:1$ operating point is therefore informative but not sufficient. It refers about 40\% of lesions and reaches 87\% sensitivity, still below a screening-oriented safety target for melanoma triage. The sensitivity-constrained thresholds give a clearer safety interpretation. If the system must maintain approximately 98\% sensitivity, the test referral rate rises to about 76\%; if the calibration target is raised to 99\%, the test referral rate rises to about 81\% while test sensitivity reaches roughly 99\%. These values are operationally expensive, but they make the tradeoff explicit rather than hiding it behind accuracy or AUC.

The uncertainty analysis suggests a practical compromise: triage should not be forced into a single binary action. A more clinically plausible policy has three zones. Lesions with low calibrated melanoma probability and low epistemic uncertainty can be auto-reassured. Lesions with high probability should be referred urgently. Lesions in between, or lesions with high MC Dropout mutual information, should be sent to manual review. In the held-out test runs, this policy auto-reassures on average 19.0\% of lesions with, on average, between one and two false negatives among auto-reassured cases, while urgent referrals are much more enriched for melanoma than the broad low-threshold referral sets. This framing fits the assignment's uncertainty requirement more naturally than using MC Dropout only as a plot, because it gives uncertainty a decision role.

The uncertainty-aware triage zone makes this practical. Using the calibration-selected 99\% sensitivity threshold as the low-risk cutoff and the 95th percentile calibration mutual information as an uncertainty cutoff, the model auto-reassures on average 353 test lesions (19.0\%), sends 1,434 lesions (77.1\%) to manual review, and marks 72 lesions (3.9\%) as urgent referrals. On average between one and two melanomas fall among the auto-reassured cases, corresponding to 99.3\% sensitivity after auto-reassurance, and the urgent-referral group has 64.4\% precision. This is a conservative workflow, but it gives MC Dropout a direct decision role.

\begin{table}[htbp]
\centering
\caption{Three-zone triage policy using calibrated probability and MC Dropout mutual information. Low-risk auto-reassurance requires both low predicted melanoma probability and low epistemic uncertainty. Values are mean (standard deviation) over three seeds; counts are the mean number of test lesions.}
\label{tab:triage-zone}
\begin{tabular}{lrr}
\toprule
Triage zone metric & Value & Count \\
\midrule
Auto-reassured lesions & 19.0\% (2.8\%) & 353 \\
Manual-review lesions & 77.1\% (2.6\%) & 1,434 \\
Urgent-referral lesions & 3.9\% (0.4\%) & 72 \\
False negatives among auto-reassured & 1.3 (0.9) & -- \\
Sensitivity after auto-reassurance & 99.3\% (0.5\%) & -- \\
Urgent-referral precision & 64.4\% (0.4\%) & -- \\
\bottomrule
\end{tabular}
\end{table}


Several limitations remain. HAM10000 is a valuable benchmark, but it is retrospective, multi-source, and partly labeled through non-histopathological procedures \citep{tschandl2018ham10000}. The ISIC challenge literature also emphasizes that algorithms with similar benchmark scores can differ in their ability to generalize to external data \citep{codella2018skin}. The present experiment repeats the analysis over three random seeds, each with its own lesion-aware split, which quantifies internal variability but is still not a substitute for repeated external validation, and the GMM baseline is deliberately simple. The calibration split is internal to the same HAM10000 training pool rather than drawn from an external cohort. The findings should therefore be interpreted as a controlled demonstration of Bayesian triage behavior rather than evidence for deployment.
...
\caption{Training loss and validation AUC over the head-training and fine-tuning stages (primary seed). Discriminative performance improves before plateauing near validation AUC 0.876.}
\label{fig:training}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{Example test predictions from the held-out test run (primary seed). The $10:1$ threshold is 0.091, while the low-risk triage threshold selected for at least 99\% calibration sensitivity is 0.006.}
\label{tab:examples}
\begin{tabular}{lllrr}
\toprule
Image ID & Diagnosis & Role & $p(\mathrm{mel})$ & Mutual information \\
\midrule
ISIC\_0031425 & \texttt{nv} & low-risk benign & 0.0001 & 0.0000 \\
ISIC\_0032970 & \texttt{mel} & urgent melanoma & 0.9331 & 0.0091 \\
ISIC\_0034085 & \texttt{mel} & near $10:1$ threshold & 0.0916 & 0.0222 \\
ISIC\_0026150 & \texttt{mel} & high uncertainty & 0.6619 & 0.0760 \\
ISIC\_0033839 & \texttt{nv} & high uncertainty & 0.1999 & 0.0737 \\
\bottomrule
\end{tabular}
\end{table}
